...
function fillTabelGlance(array) {
  ...
  srnumb.innerHTML = statistics.number_of_republicans;
  sratt.innerHTML = statistics.votes_republicans.toFixed(2) + "%";
  sdnumb.innerHTML = statistics.number_of_democrats;
  sdatt.innerHTML = statistics.votes_democrats.toFixed(2) + "%";
  sinumb.innerHTML = statistics.number_of_independents;
  siatt.innerHTML = statistics.votes_independents.toFixed(2) + "%";
  stnumb.innerHTML = statistics.total_number;
  statt.innerHTML = statistics.total_average.toFixed(2) + "%";
...
function leastEngaged(array) {
  table = document.getElementById("LeastEngagedSenate");
  function compare(a, b) {
    for (var i = 0; i < array.length; i++) {
      if (a.missed_votes_pct > b.missed_votes_pct) {
        return -1;
      }
      if (a.mmissed_votes_pct < b.missed_votes_pct) {
        return 1;
      }
      return 0;
    }
...
function mostEngaged(array) {
  ...
  var j = 0;
  var x = Math.round(array.length * 0.1);
  var table = document.getElementById("mostEngagedSenate");
